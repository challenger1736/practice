// 식별자 : 절대적으로 식별할 수 있는 고정값 + 중복 없어야 한다.
// -!!! : 배열의 인덱스는 절대적인 식별자 역할 할 수 없다.
// - 주민등록번호, 사번, 학번, 제품번호, ISBN 등등
// { 주민등록번호 : 870625-1.... , 이름: 김현수, 주소: 인천, 직업: 강사}

const 전국영화관 = [
    {지점코드 : 1, 지점명: '안산센트럴락', 주소 : '고잔로 108', 영화 : [ {영화이름 : '서울의봄', 타임테이블 : [ {관 : '1관', 시간: '12:00'},{관 : '3관', 시간: '12:00'} ] },
                                                                    {영화이름 : '노량', 타임테이블 : [ {관 : '1관', 시간: '12:00'},{관 : '3관', 시간: '12:00'} ] } ]
    },
...
function 영화출력(선택한지점코드){    //함수 선언 [실행 조건 : 영화관을 클릭했을때]
    //1. 어디에
    ...
    let html = "";
        // 1. 선택한 지점코드 찾기.
        for(let i =0; i < 전국영화관.length ; i++ ){
            // 만약에 전국영화관내 지점코드 중에 내가 선택한 지점코드와 같으면
            if(전국영화관[i].지점코드 == 선택한지점코드){
                // 2. 찾았으면 해당 지점의 영화를 출력
                for(let j = 0 ; j<전국영화관[i].영화.length ; j++){
                    html += `<span style="margin:10px"; onclick="시간출력(${선택한지점코드},${j})">${전국영화관[i].영화[j].영화이름}</span>`
                }
            }
        }
    //3. 출력
...
function 시간출력(선택한지점코드, 영화인덱스){
    ...
    let html = '';
        for(let i = 0  ; i < 전국영화관.length ; i ++){
            if(전국영화관[i].지점코드 == 선택한지점코드){
                console.log(전국영화관[i].영화[영화인덱스].타임테이블);
                for(let j =0; j<전국영화관[i].영화.length ; j++ ){
                    if(전국영화관[i].영화[j].영화이름 == 선택한지점코드){
                        for (let t = 0 ; t < 전국영화관[i].영화[j].시간테이블.length; t++){
                            html +=  `<span style="margin:10px"; >${전국영화관[i].영화[j].타임테이블[t].관}${전국영화관[i].영화[j].타임테이블[t].시간}</span>`
                        }
                    }
                }
            }
