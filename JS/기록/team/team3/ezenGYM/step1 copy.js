...
const 목록 = [];
// const 이름목록 = []; /* 유효성 검사를 위한 어쩔수 없는 배열 */ 인줄 알았으나 findIndex()로 객체내 속성 겹치는지 확인 가능 ★★★★★★ findIndex ★★★★★★
...
function 등록함수(){ console.log('등록함수() 실행');
    ...
    const nIndex = 목록.findIndex(i => i.이름 == 이름);  // 배열이름 비교 
    ...
    if(!(이름 == namebin)){     // 빈칸이 입력 됬는지 비교
        ...
        목록.push(  { 이름 : 이름, 팔굽혀펴기 : 팔굽혀펴기, 윗몸일으키기 : 윗몸일으키기, 턱걸이 : 턱걸이})
...
let result = ""
let 총점 = Number(목록[a].팔굽혀펴기)+Number(목록[a].윗몸일으키기)+Number(목록[a].턱걸이)
...
function 수정함수(){    // 수정
    ...
    const nIndex = 목록.findIndex(i=>i.이름 == 수정이름);  // 배열이름 비교 
    ...
    if(nIndex >= 0){        // 똑같은 이름이 없으면 -1  있으면 해당 인덱스값 
        ...
        if(!(팔굽혀펴기 == '')){    // 팔굽혀펴기 값 수정
            목록[nIndex].팔굽혀펴기 = 팔굽혀펴기;
        }else{
            목록[nIndex].팔굽혀펴기 = 목록[nIndex].팔굽혀펴기
        }

        if(!(윗몸일으키기 == '')){  // 윗몸일으키기 값 수정
            목록[nIndex].윗몸일으키기 = 윗몸일으키기;
        }else{
            목록[nIndex].윗몸일으키기 = 목록[nIndex].윗몸일으키기
        }

        if(!(턱걸이 == '')){        // 턱걸이 값 수정
            목록[nIndex].턱걸이 = 턱걸이;
        }else{
            목록[nIndex].턱걸이 = 목록[nIndex].턱걸이
        }
        출력함수()  //출력
    ...
    let html = '';
        for( let i = 0; i<목록.length; i++){
            html +=  `<div class="output">        <!-- JS 출력함수() 부분 가이드 -->
                        <div class="name">${목록[i].이름}</div>
                        <div class="list_1">${목록[i].팔굽혀펴기}개</div>
                        <div class="list_2">${목록[i].윗몸일으키기}개</div>
                        <div class="list_3">${목록[i].턱걸이}개</div>
                        <div class="total">${Number(목록[i].팔굽혀펴기)+Number(목록[i].윗몸일으키기)+Number(목록[i].턱걸이)}점</div>
